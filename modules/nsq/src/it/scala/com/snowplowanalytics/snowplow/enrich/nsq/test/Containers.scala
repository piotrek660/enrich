...
object Containers {
  ...
  private def enrich[F[_] : Sync](
    ...
    Resource.make(
      Sync[F].delay {
        val config = Config.base64EnrichConfig(
          lookupHost = topology.lookup1.networkAlias,
          lookupPort = topology.lookup1.httpPort,
          nsqdHost = topology.nsqd1.networkAlias,
          nsqdPort = topology.nsqd1.tcpPort,
          inputTopic = topology.sourceTopic,
          goodOutputTopic = topology.goodDestTopic,
          badOutputTopic = topology.badDestTopic
        )
        val container = GenericContainer(
